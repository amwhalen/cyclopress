...
function cy_get_summary($compare=false,$year=false) {
	...
	$avg_time = ($stats['total_rides'] == 0) ? : cy_minutes_human($stats['total_time']/$stats['total_rides']);
...
function cy_get_summary_compare($year=false) {
	...
	$average_ride_time = ($stats['total_rides'] == 0) ? : cy_minutes_human($stats['total_time']/$stats['total_rides']);
	$average_ride_time_now = ($stats_now['total_rides'] == 0) ? : cy_minutes_human($stats_now['total_time']/$stats_now['total_rides']);
	...
	<form method="post" action="options.php">
		...
		<table class="widefat">
			<tr valign="top">
				...
				<td>
					<select name="cy_unit">
						<option value="miles"<?php if (get_option('cy_unit')=='mile') { echo ' selected="selected"'; } ?>>US (miles, feet, pounds)</option>
						<option value="kilometer"<?php if (get_option('cy_unit')=='kilometer') { echo ' selected="selected"'; } ?>>SI (kilometers, meters, kg)</option>
					</select>
...
function cy_manage_calendar_page() {
	...
	<div class="wrap">
		...
		echo cy_admin_navigation('manage','manage_calendar');
			...
			for ($rows = 0; $rows < $rows_in_month; $rows++) {
				...
				<tr>
					...
					for ($d = 0; $d < 7; $d++) {
						...
						if (($rows == 0 && $d < $start_day_of_week) || $day_index > $days_in_month) {
